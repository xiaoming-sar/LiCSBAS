...
with open(gap_infofile, "w") as f:
    print('{}-{},{:.2f}'.format(final_imdates[0],final_imdates[-1],period), file=f) #first/last dates and period [years]
...
pngfile = os.path.join(netdir, 'network121_nobad_nogap_1.png')
plot_lib.plot_network(ifgdates_all, bperp_all, bad_ifg_all, pngfile, plot_bad=True)

pngfile_nogap = os.path.join(netdir, 'network121_nobad_nogap.png')
